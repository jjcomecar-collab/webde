...
<div class="card mb-4">
    <div class="card-header">
        {{ $editar ? 'Editar Administrador' : 'Registrar Administrador' }}
    </div>

    <div class="card-body">
        <form action="{{ $editar ? route('administradorfun.update', $editar->id) : route('administradorfun.store') }}"
              method="POST"
            ...
            <div>
                ...
                @if($editar)
                    <a href="{{ route('administradorfun.index') }}" class="btn btn-warning">
                        Cancelar
                    </a>
                @endif
            </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="d-flex gap-1">
                        <a href="{{ route('administradorfun.edit', $r->id) }}" class="btn btn-warning btn-sm">
                            Editar
                        </a>

                        <form action="{{ route('administradorfun.destroy', $r->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
